...
1. **Dual Interfaces**: Farmify provides separate interfaces for consumers and sellers, accessible through the navbar with options for SignUp and SignIn including email verification for the created account.
...
To run Farmify locally, ensure you have NodeJS and MongoDB installed. Follow these steps:
...
2. **Frontend Setup**:
    - Navigate to the Farmify `client` folder.
    ...
    - Add the following environment variables to the client `.env` file:
        ```plaintext
        VITE_Farmify_API = "http://Farmify-backend.vercel.app/" 
...
3. **Backend Setup**:
    - Navigate to the Farmify `server` folder.
...
By following these steps, you'll have the Farmify application running locally on your machine. Adjust configurations as needed for your development environment.

## Contribution

Farmify welcomes contributions from the community. Feel free to open issues or submit pull requests to help improve the platform.
